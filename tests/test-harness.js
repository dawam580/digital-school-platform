 ...
 *
 * Provides headless browser DOM emulation, local storage persistence mocking,
 * Web Audio mock, canvas confetti mock, assertion library, and BDD test runner.
 */

// In-Memory Storage Mock simulating browser localStorage
...
// Global Browser Environment Emulation
export function setupBrowserEnvironment() {
  ...
  const mockWindow = {
    ...
    print: () => { mockWindow._lastPrinted = Date.now(); },
  ...
  const mockDocument = {
    body: {
      ...
      removeChild(node) {
        if (node && node.id) elements.delete(node.id);
        return node;
      }
    },
    getElementById(id) {
      if (id && id.startsWith('otp-input-')) {
        return {
          id,
          focus: () => { mockDocument._lastFocused = id; }
        };
      }
  ...
  class MockFileReader {
    readAsDataURL(file) {
      setTimeout(() => {
        this.result = `data:image/png;base64,mock_${file.name || 'avatar'}`;
        if (this.onload) this.onload({ target: { result: this.result } });
      }, 5);
    }
  }

  // Bind to global Node.js scope
  globalThis.window = mockWindow;
  globalThis.document = mockDocument;
  globalThis.localStorage = localStorage;
  globalThis.sessionStorage = mockWindow.sessionStorage;
  ...
  return { window: mockWindow, document: mockDocument, localStorage };
}
  ...
  return {
    ...
    toContain(item) {
      if (typeof actual === 'string') {
        ...
        const found = actual.some(x => JSON.stringify(x) === JSON.stringify(item) || x === item);
        if (!found) {
    ...
    toThrow(expectedMsg) {
      if (typeof actual !== 'function') {
        throw new Error(`toThrow requires a function but got ${typeof actual}`);
      }
      let threw = false;
      let error = null;
      try {
      ...
      if (expectedMsg && error) {
        const msg = error.message || String(error);
        if (typeof expectedMsg === 'string' && !msg.includes(expectedMsg)) {
          throw new Error(`Expected error message to contain "${expectedMsg}" but got "${msg}"`);
        } else if (expectedMsg instanceof RegExp && !expectedMsg.test(msg)) {
          throw new Error(`Expected error message to match ${expectedMsg} but got "${msg}"`);
        }
      }
    }
  };
...
// BDD Test Suite Runner Engine
export class TestRunner {
  constructor(suiteName) {
    this.suiteName = suiteName;
    this.currentDescribe = suiteName;
    this.tests = [];
  ...
  describe(name, fn) {
    ...
    this.currentDescribe = `${prevDescribe} > ${name}`;
    try {
      fn();
    } finally {
  ...
  test(name, fn) {
    this.tests.push({
      describeName: this.currentDescribe,
      testName: name,
      fn
    });
  }
  ...
  async run() {
    ...
    for (const t of this.tests) {
      const testFullName = `${t.describeName} → ${t.testName}`;
      try {
        ...
        this.results.push({ name: testFullName, status: 'pass' });
        passed++;
      } catch (err) {
        this.results.push({ name: testFullName, status: 'fail', error: err });
        failed++;
      }
    }
...
// Canonical Seed Data Definitions (Exact mirrors of db.ts)
...
export const SEED_DAILY_REPORT = {
  id: 'rep-today',
  studentId: 'std-1',
  studentName: 'ريان فهد العتيبي',
  date: '2026-09-01',
...
// Database Service Implementation with localStorage synchronization
export const STORAGE_KEY_STUDENTS = 'madrasa_db_students_v2';
export const STORAGE_KEY_CLASSES = 'madrasa_db_classes_v2';
export const STORAGE_KEY_NOTIFICATIONS = 'madrasa_db_notifications_v2';
export const STORAGE_KEY_REPORTS = 'madrasa_db_reports_v2';

export class DatabaseService {
  getStudents() {
    try {
      const data = globalThis.localStorage.getItem(STORAGE_KEY_STUDENTS);
  ...
  saveStudents(students) {
    try {
      globalThis.localStorage.setItem(STORAGE_KEY_STUDENTS, JSON.stringify(students));
    } catch {}
  }

  getClasses() {
    try {
      const data = globalThis.localStorage.getItem(STORAGE_KEY_CLASSES);
      if (data) return JSON.parse(data);
    } catch {}
    this.saveClasses(SEED_CLASSES);
    return JSON.parse(JSON.stringify(SEED_CLASSES));
  }

  saveClasses(classes) {
    try {
      globalThis.localStorage.setItem(STORAGE_KEY_CLASSES, JSON.stringify(classes));
    } catch {}
  }

  getNotifications() {
    try {
      const data = globalThis.localStorage.getItem(STORAGE_KEY_NOTIFICATIONS);
      if (data) return JSON.parse(data);
    } catch {}
  ...
  saveNotifications(notifs) {
    try {
      globalThis.localStorage.setItem(STORAGE_KEY_NOTIFICATIONS, JSON.stringify(notifs));
    } catch {}
  }

  getDailyReport() {
    try {
      const data = globalThis.localStorage.getItem(STORAGE_KEY_REPORTS);
      if (data) return JSON.parse(data);
    } catch {}
    this.saveDailyReport(SEED_DAILY_REPORT);
    return JSON.parse(JSON.stringify(SEED_DAILY_REPORT));
  }

  saveDailyReport(report) {
    try {
      globalThis.localStorage.setItem(STORAGE_KEY_REPORTS, JSON.stringify(report));
    } catch {}
  }
...
// School State Simulator for high-fidelity workflow testing
export class SchoolStateSimulator {
  constructor() {
    ...
    this.selectedStudent = this.students[0] || SEED_STUDENTS[0];
    this.classes = this.db.getClasses();
    this.notifications = this.db.getNotifications();
    this.dailyReport = this.db.getDailyReport();
  }
  ...
  recordSound(type) {
    if (this.soundEnabled) {
      this.soundCalls.push({ type, time: Date.now() });
    }
  }
  ...
  updateAttendance(studentId, status, note) {
    ...
    this.db.saveStudents(this.students);

    const updatedSelected = this.students.find(s => s.id === this.selectedStudent.id);
    if (updatedSelected) {
      this.selectedStudent = updatedSelected;
    }

    if (status === 'present') this.recordSound('playTap');
    else if (status === 'unexcused') this.recordSound('playAlert');
    else this.recordSound('playTap');
  }
  ...
  markAllPresent(classId) {
    ...
    this.students = this.students.map(s => ({
      ...s,
      status: 'present',
      recentAttendance: [{ date: today, status: 'present' }, ...(s.recentAttendance || []).filter(r => r.date !== today)]
    }));
  ...
  linkStudent(codeOrId) {
    const trimmed = (codeOrId || '').trim();
    const found = this.students.find(
      s => (s.linkCode && s.linkCode.toLowerCase() === trimmed.toLowerCase()) ||
           s.studentNumber === trimmed ||
    ...
    this.recordSound('playAlert');
    return false;
  }

  addBehaviorPoint(studentId, point) {
    this.students = this.students.map(s => {
      if (s.id === studentId) {
        const newTotal = Math.max(0, (s.behaviorPointsTotal || 0) + point.points);
        return {
          ...s,
  ...
  updateStudentAvatar(studentId, avatarUrl) {
    ...
    if (this.selectedStudent.id === studentId) {
      this.selectedStudent = { ...this.selectedStudent, avatar: avatarUrl };
    }
    this.recordSound('playSuccess');
  }

  addNotification(title, message, category, studentName) {
    const newNotif = {
    ...
    this.db.saveNotifications(this.notifications);
  }
